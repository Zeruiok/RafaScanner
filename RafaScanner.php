...
function ui_print($text, $color = "") {
    echo ($color !== "" ? $color : "") . $text . C_RESET . PHP_EOL;
}
...
function rafa_banner() {
    clear_screen();
    $art = <<<ART
 ____        _           ____  _____ _____ 
|  _ \  __ _| |_ __ _   / ___|| ____| ____|
| | | |/ _` | __/ _` |  \___ \|  _| |  _|  
| |_| | (_| | || (_| |   ___) | |___ | |___ 
...
function detectarBypassShell() {
    ...
    foreach ($arquivosConfig as $arquivo) {
     ...
     if (empty($resultadoGitHelp) || strpos($resultadoGitHelp, 'usage: git') === false) {
         $comandoTestClone = 'adb shell "cd /tmp 2>/dev/null || cd /data/local/tmp; timeout 5 git clone https://github.com/RafaSS-Star/RafaScanner test-repo 2>&1 | head -3"';
    ...
    echo $bold . $branco . "[•] Testando bloqueio de comandos pkg...\n";
        ...
        if ($opcaoscanner == "0") {
            ...
            $comandoDispositivos = shell_exec("adb devices 2>&1");
                ...
                if (!empty($resultadoSu) && strpos($resultadoSu, 'No su program found') !== false) {
                    ...
                    echo $bold . $vermelho . "[!] Root detectado no dispositivo.\n\n";
                }
            ...
            if ($scriptsAtivos !== null && trim($scriptsAtivos) !== '') {
                echo $bold . $amarelo . "[!] Scripts detectados rodando em segundo plano! Cancelando scanner...\n";
            ...
            if (preg_match('/up (\d+) min/', $comandoUPTIME, $filtros)) {
                ...
                echo $bold . $amarelo . "[•] Dispositivo não reiniciado\n\n";
            }
            ...
            echo $bold . $amarelo . "[•] Checando se o replay foi passado...\n";
                ...
                if (empty($arquivos)) {
                    $motivos[] = " - Nenhum arquivo encontrado na pasta MReplays";
                }
                ...
                echo $bold . $branco . "[+] Após verificar no jogo se o acusado está de Holograma  verifique os horários do Shaders e OBB e compare  caso esteja muito diferente as datas, aplique o W.O!\n\n";
            ...
            echo $bold . $branco . "[+] Verificando scripts ativos em segundo plano...\n";
            $comandoScripts = 'adb shell "pgrep -a bash | awk \'{\$1=\"\"; sub(/^ /,\"\"); print}\' | grep -vFx \"/data/data/com.termux/files/usr/bin/bash -l\""';
            $scriptsAtivos = shell_exec($comandoScripts);
            ...
            echo $bold . $fverde . "[•] Nenhum app ativo detectado.\n";
            echo $bold . $branco . "[•] Finalizando sessões bash desnecessárias...\n";
            $comandoKillBash = 'adb shell "current_pid=\$\$; for pid in \$(pgrep bash); do [ \"\$pid\" -ne \"\$current_pid\" ] && kill -9 \$pid; done"';
            shell_exec($comandoKillBash);
            echo $bold . $fverde . "[i] Sessões desnecessárias finalizadas.\n\n";
            ...
            echo $bold . $azul . "[+] Checando se o replay foi passado...\n";
                ...
                echo $bold . $branco . "[*] Verifique o horário da  alteração, se for após a partida, aplique o w.o\n\n";
                ...
                if (strpos($resultadoStat, 'File:') !== false) {
                    ...
                    if ($matchAccess && $matchModify && $matchChange) {
                        ...
                        if ($dataAccess === $dataModify && $dataModify === $dataChange) {
                            echo $bold . $vermelho . "[!] Possível bypass detectado! - Datas idênticas\n";
                            echo $bold . $vermelho . "[!] Data: " . $dataModifyFormatada . "\n";
                        } elseif ($dataModify === $dataChange) {
                            echo $bold . $vermelho . "[!] Modificação da pasta: " . $dataModifyFormatada . "\n";
                        } else {
                            echo $bold . $vermelho . "[×] Discrepância encontrada\n";
                            $dateAccess = DateTime::createFromFormat('Y-m-d H:i:s.u', $dataAccess);
                            $dateChange = DateTime::createFromFormat('Y-m-d H:i:s.u', $dataChange);
                            if ($dateAccess && $dateChange) {
                                echo $bold . $branco . "[i] Acesso: " . $dateAccess->format('d-m-Y H:i:s') . "\n";
                                echo $bold . $branco . "[i] Modificação: " . $dataModifyFormatada . "\n";
                                echo $bold . $branco . "[i] Mudança: " . $dateChange->format('d-m-Y H:i:s') . "\n";
                            }
                        }
                    } else {
                        echo $bold . $vermelho . "[×] Não foi possível extrair datas. Resultado:\n";
                ...
                echo $bold . $vermelho . "[+] Caso a pasta 'android' esteja modificada após o fim da partida, aplique o W.O!\n\n";
                ...
                if ($resultadoArquivos !== '') {
                    ...
                    if (!$modificacaoDetectada) {
                        echo $bold . $fverde . "[•] Nenhuma alteração suspeita encontrada nos arquivos.\n\n";
                    }
                } else {
                    echo $vermelho . "[!] Sem itens baixados! Verifique se a data é após o fim da partida!\n\n";
                }
                ...
                echo $bold . $amarelo . "[•] Checando OBB...\n";
                ...
                echo $bold . $vermelho . "[!] Após verificar no jogo  se o acusado está de Holograma verifique os horários do Shaders e OBB e compare caso esteja muito diferente as datas, aplique o W.O!\n\n";

                echo $bold . $branco . "\n\n\t \n";
                echo $bold . $fverde . "\t                 Credits:KellerSS...\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n";
